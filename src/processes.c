...
static void ps_submit_proc_list (procstat_t *ps)
{
	...
	vl.values[0].gauge = ps->pid;
	vl.values_len = 1;
	plugin_dispatch_values (&vl);

	sstrncpy (vl.type, "ps_ppid", sizeof (vl.type));
	vl.values[0].gauge = ps->ppid;
	vl.values_len = 1;
	plugin_dispatch_values (&vl);

	sstrncpy (vl.type, "ps_runtime", sizeof (vl.type));
	vl.values[0].gauge = ps->runtime_secs;
	vl.values_len = 1;
	plugin_dispatch_values (&vl);
...
static _Bool config_threshold_exceeded(procstat_t *ps)
{
 ...
 void ps_find_cpu_delta(procstat_t *ps, unsigned long *out_userd, unsigned long *out_sysd)
{
	procstat_t *ps_ptr;
